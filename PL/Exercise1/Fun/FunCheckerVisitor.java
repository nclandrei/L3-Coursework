...
//// BEGINNING OF EXTENSION
    ...
    public Type visitSwitch(FunParser.SwitchContext ctx) {
        ...
        for (FunParser.Fun_caseContext switchCase : cases) {          
           Type caseType = visit(switchCase);
           checkType(t, caseType, ctx);
           String guard  = switchCase.v1.getText();
           if (guards.contains(guard)) {
               reportError("Guard " + guard + " has already been used.", ctx);
           }
           else {
               guards.add(guard);
           }
